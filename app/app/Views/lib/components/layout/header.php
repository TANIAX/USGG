...
<header x-data="{ open: false, hide_menu: ((window.innerWidth > 0) ? window.innerWidth : screen.width) > 768 ? false : true}" 
  ...
  <div class="flex gap-x-6 bg-blue-400 px-6 py-2.5 sm:px-3.5 sm:before:flex-1" style="background-color: #03497A;">
    <div class="sm:w-full cursor-pointer md:hidden" @click="open = !open">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="w-6 h-6" x-bind:class="open ? 'hidden' : ''">
    ...
    <div class="flex flex-1 items-center justify-end md:px-12">
      ...
      <?php else : ?>
        <a href="/auth/logout" class="text-white hover:text-gray-200 px-2 uppercase text-xs tracking-widest sofia font-bold">
